...
selector = comm:["--"]* '@' selName:charsRaw _ selValue:charsRaw {
	function temp(comm, selName, selValue) {
		if (comm != "") {
			return "null";
		} else {
			return {type: selName, value: selValue};
		}
	}
	return temp(comm, selName, selValue)
}